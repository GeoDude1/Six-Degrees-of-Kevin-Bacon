...
public class ActorsGraph {
	...
	public void addEdge(String actor1, String actor2) {
		//First, it gets the nodes of the actors
		Node node1 = actors.get(actor1);
		Node node2 = actors.get(actor2);

		//Then, it adds the relation between the actors
	...
	public ArrayList<String> shortestPath(String actor1, String actor2) {
		...
		//Now, it starts adding the actor1 node to the List and adding it to the parent Map
		Node start = actors.get(actor1);
		temp.add(start);
		parents.put(actor1, null);

		//Now, it loops as long as there are nodes to visit
		while (temp.size() > 0) {
			...
			for (int i = 0; i < knownActors.size(); i++) {
				...
				//If the current now was visited already, then it continues with the next known actor
				if (visited) {
					continue;
				}
				//If not, it adds it to the List of temp nodes to be visited
				else {
					temp.add(nextActor);
					...
					//it is looking for. If it is, it returns an ArrayList with the actual path
	...
	private class Node {
		...
		public void addRelation(Node actor) {
			//it checks if the relation doesn't exist already
			boolean exist = false;
			for(Node n : known) {
			...
			//then, it adds it only if it doesn't exist
			if(!exist) {
				known.add(actor);
			}
		}
